...
from distroforge.ui.artifact_release_paths import resolve_artifact_release_paths
...
def load_artifact_defaults_action(window) -> None:
    ...
    window.artifacts_build_run_id_edit.clear()
    window.artifacts_boot_run_id_edit.clear()
...
def run_release_readiness_action(window) -> None:
    ...
    def _work():
        return ReleaseReadinessService().check(
            paths.iso,
            paths.product_output_dir,
        )
...
def run_release_gate_action(window) -> None:
    ...
    def _done(report):
        _remember_release_runs(window, report)
        window.artifacts_view.setPlainText(report.render_text())
...
def create_publish_bundle_action(window) -> None:
    ...
    def _done(report):
        _remember_release_runs(window, report.gate)
        window.artifacts_view.setPlainText(report.render_text())
...
def _remember_release_runs(window, report) -> None:
    build_run_id = getattr(report, "build_run_id", None)
    boot_run_id = getattr(report, "boot_run_id", None)
    if build_run_id:
        window.artifacts_build_run_id_edit.setText(build_run_id)
    if boot_run_id:
        window.artifacts_boot_run_id_edit.setText(boot_run_id)


def run_qemu_smoke_plan_action(window) -> None:
    plan = QemuSmokePlanner().plan(resolve_artifact_release_paths(window).iso)
