from __future__ import annotations

from dataclasses import dataclass
from pathlib import Path

from .chroot import ChrootService
from .command import CommandRunner
from .fsops import FileSystemOps


@dataclass
class KioskOptions:
    enabled: bool = False
    browser: str = "firefox"
    url: str = "about:blank"
    user: str = "ubuntu"


class KioskService:
    def __init__(
        self,
        runner: CommandRunner,
        root: Path,
        options: KioskOptions,
        use_sudo: bool = True,
    ) -> None:
        self.runner = runner
        self.root = root
        self.options = options
        self.use_sudo = use_sudo
        self.fs = FileSystemOps(runner, use_sudo)

    def apply(self) -> None:
        if not self.options.enabled:
            return
        ChrootService(self.runner, self.root, self.use_sudo).run(
            "apt-get",
            "-y",
            "install",
            self.options.browser,
            "unclutter",
        )
        desktop = self.root / "home" / self.options.user / ".config" / "autostart" / "distroforge-kiosk.desktop"
        self.fs.write_text(
            desktop,
            "[Desktop Entry]\nType=Application\nName=Kiosk\n"
            f"Exec={self.options.browser} --kiosk {self.options.url}\n",
            "Write kiosk autostart",
        )
