...
_WEIGHTS: dict[BuildPhase, float] = {
    ...
    BuildPhase.FINALIZE_PACKAGES: 2.0,
    BuildPhase.PACKAGE_EVIDENCE: 6.0,
    ...
    BuildPhase.ROOTFS_EVIDENCE_CAPTURE: 12.0,
    BuildPhase.REPACK_FILESYSTEM: 18.0,
    BuildPhase.ROOTFS_EVIDENCE_VERIFY: 20.0,
...
def build_phase_sequence(*, source_mode: str, run_preview: bool) -> tuple[PlannedStep, ...]:
    ...
    if source_mode == "bootstrap":
        source_steps = [
            _step(
                ...
                "Create minimal rootfs and live ISO tree",
            ),
        ]
    else:
        source_steps = [
            _step(
                BuildPhase.EXTRACT_ISO, "Extract ISO", "Extract the source ISO into the work tree"
            ),
            _step(
    ...
    steps: list[PlannedStep] = [
        _step(
            BuildPhase.VALIDATE,
            "Validate project",
            "Check source ISO, packages, repositories and host tools",
        ),
        _step(
            BuildPhase.CONSISTENCY,
            "Check remix consistency",
            "Detect risky desktop, boot, Secure Boot and release-track combinations",
        ),
        _step(
            BuildPhase.POLICY,
            "Apply beginner-safe policy",
            "Optionally enforce strict guardrails before the build starts",
        ),
        _step(
            BuildPhase.COMPATIBILITY,
            "Check release compatibility",
            "Check release support, PPA intent and source mode constraints",
        ),
        _step(
            BuildPhase.IMPORT_SCRIPTS,
            "Import legacy scripts",
            "Optionally import old customization scripts as chroot hooks",
        ),
        _step(
            BuildPhase.DIFF_PREVIEW,
            "Preview changes",
            "Summarize packages, snaps, services and high-risk flags",
        ),
        _step(
            BuildPhase.PREPARE,
            "Prepare workspace",
            "Create the build workspace and output directory",
        ),
        *source_steps,
        _step(
            BuildPhase.BRANDING,
            "Debrand source identity",
            "Scan and rewrite source identity text for redistribution",
        ),
        _step(
            BuildPhase.CONFIGURE_APT,
            "Configure repositories",
            "Write apt sources for the target release",
        ),
        _step(
            BuildPhase.APT_CACHE,
            "Configure apt cache",
            "Optionally configure package cache or proxy",
        ),
        _step(
            BuildPhase.PPA,
            "Configure verified PPAs",
            "Resolve Launchpad signing keys and add signed-by entries",
        ),
        _step(
            BuildPhase.RELEASE_TRACK,
            "Configure release track",
            "Optionally enable a devel/rolling-like apt track with pinning",
        ),
        _step(
            BuildPhase.SYSTEM_SYNC,
            "Sync system packages",
            "Optionally run an apt-native full system sync with fallback",
        ),
        _step(
            BuildPhase.AUTODRIVERS, "Auto-install drivers", "Optionally run ubuntu-drivers install"
        ),
        _step(
            BuildPhase.APPLY_PACKAGES,
            "Apply package plan",
            "Install/remove requested packages inside the target root",
        ),
        _step(
            BuildPhase.DESKTOP_SOURCE,
            "Build desktop from source",
            "Optionally build selected DE upstream sources into local .deb packages",
        ),
        _step(
            BuildPhase.INSTALL_SNAPS,
            "Install snaps",
            "Optionally install snaps with pinned channels",
        ),
        _step(
            BuildPhase.SIZE_ANALYSIS,
            "Analyze image size",
            "Optionally record largest installed packages",
        ),
        _step(
            BuildPhase.VULN_SCAN,
            "Scan packages for known CVEs",
            "Match the planned package set against the offline advisory database",
        ),
        _step(
            BuildPhase.SNAPSHOT,
            "Create rollback snapshot",
            "Optional rollback snapshot after package application (after-apt)",
        ),
        _step(
            BuildPhase.CUSTOMIZE_SYSTEM,
            "Apply ISO personalization",
            "Configure desktop, display manager, autologin, wallpaper and locale",
        ),
        _step(
            BuildPhase.BRANDING,
            "Apply branding",
            "Apply logo, GRUB, Plymouth and release identity assets",
        ),
        _step(
            BuildPhase.USERS,
            "Configure users and groups",
            "Optionally create default users and group memberships",
        ),
        _step(
            BuildPhase.SYSTEMD,
            "Configure systemd services",
            "Optionally enable, disable or mask services",
        ),
        _step(
            BuildPhase.NETWORK,
            "Configure network",
            "Optionally write Netplan, DNS and apt proxy configuration",
        ),
        ...
        _step(
            BuildPhase.SNAPSHOT,
            "Create rollback snapshot",
            "Optional rollback snapshot after customization (after-customize)",
        ),
        _step(
            BuildPhase.KERNEL_MODULE,
            "Build kernel payload",
            "Optionally fetch kernel sources, compile module and refresh boot assets",
        ),
        _step(
            BuildPhase.SECURE_BOOT,
            "Secure Boot workflow",
            "Optionally prepare module signing and MOK checks",
        ),
        _step(
            BuildPhase.REPRODUCIBLE,
            "Pin reproducible build inputs",
            "Optionally refuse a reproducible build whose epoch or archive snapshot is not pinned",
        ),
        _step(
            BuildPhase.RUN_HOOKS,
            "Run customization hooks",
            "Run pre-build, chroot and post-build hook scripts when present",
        ),
        _step(
            BuildPhase.FINALIZE_PACKAGES,
            "Finalize package set",
            "Run the guarded final autoremove before sealing the dpkg inventory",
        ),
        _step(
            BuildPhase.PACKAGE_EVIDENCE,
            "Seal package inputs",
            "Copy signed repository metadata, keyrings and every consumed .deb into immutable run evidence",
        ),
        _step(
            BuildPhase.SANITIZE_TARGET,
            "Sanitize target",
            "Clean caches, logs, histories, temporary files and machine identity",
        ),
        _step(
            BuildPhase.SNAPSHOT,
            "Create rollback snapshot",
            "Optional rollback snapshot after sanitize (after-sanitize)",
        ),
        _step(
            BuildPhase.HEALTH,
            "Beginner-safe health report",
            "Score consistency and guardrail status before final assembly",
        ),
        _step(
            BuildPhase.AUTOINSTALL,
            "Generate autoinstall",
            "Optionally write Subiquity autoinstall.yaml",
        ),
        _step(
            BuildPhase.SEEDS,
            "Write seeds",
            "Write Ubuntu-style seed and requested package manifests",
        ),
        _step(
            BuildPhase.UPDATE_METADATA,
            "Update ISO metadata",
            "Regenerate Casper package manifests and filesystem size",
        ),
        _step(
        ...
        _step(
            ...
            "Rebuild squashfs with the configured compression",
        ),
        _step(
            BuildPhase.ROOTFS_EVIDENCE_VERIFY,
            "Verify packed rootfs identity",
            "Extract the FD-witnessed SquashFS and compare it to the pre-pack manifest",
        ),
        _step(
            BuildPhase.UPDATE_CHECKSUMS,
            "Update ISO checksums",
            "Regenerate md5sum.txt after the new squashfs is written",
        ),
        _step(BuildPhase.REBUILD_ISO, "Rebuild ISO", "Run xorriso to create the output ISO"),
        _step(
            BuildPhase.PREBUILD_VM,
            "Run prebuild VM lab",
            "Optionally boot the ISO in an observable QEMU lab",
        ),
        _step(
            BuildPhase.RELEASE_ARTIFACTS,
            "Write release artifacts",
            "Generate SHA256SUMS, BUILDINFO and optional GPG signature",
        ),
        _step(
            BuildPhase.BOOTCHECK,
            "Boot smoke test",
            "Optionally run a minimal headless QEMU boot check",
        ),
        _step(
            BuildPhase.QEMU_SCREENSHOT,
            "Capture QEMU screenshot",
            "Optionally capture a boot screenshot for visual QA",
        ),
        _step(
            BuildPhase.HTML_REPORT,
            "Write HTML report",
            "Write a human-readable build report for beginners and reviewers",
        ),
        _step(
            BuildPhase.QA_MATRIX,
            "Run QA boot matrix",
            "Optionally run BIOS/UEFI/live/install QEMU scenarios",
        ),
    ]
    if run_preview:
